...
- `skeleton/`: the `salix/skeleton` project template for new sites
...
**Starting a new site** — use the **`salix/skeleton`** project template in `skeleton/`:
...
composer install    # also installs the prebuilt admin UI via salix:admin:install
```

The skeleton ships its own Docker setup (no Node tooling for the admin — only a small
sass build for the site theme), compose files, a shared-hosting `bin/package-release`,
and a README with the site workflows. **The admin UI arrives prebuilt** with the bundle
(`cms-bundle/public/admin`, installed to the site's `public/admin/` by
`salix:admin:install`, which runs automatically on `composer install/update`).

The `--repository`/`--stability=dev` flags and the `repositories.salix` entry are
temporary until the packages are published (next step): a CI job will subtree-split
`cms-bundle/` and `skeleton/` into read-only repos, release tags will build and commit
the admin UI into the bundle, and Packagist will serve both — reducing the above to
