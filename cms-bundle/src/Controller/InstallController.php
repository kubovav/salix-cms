...
use Salix\Cms\Install\Installer;
use Salix\Cms\Install\InstallException;
use Salix\Cms\Install\InstallRequest;
...
final class InstallController extends AbstractController
{
    ...
    public function install(Request $request, Installer $installer, ValidatorInterface $validator): Response
    {
        if ($installer->isLocked()) {
            return $this->render('@SalixCms/install/locked.html.twig', response: new Response(status: Response::HTTP_FORBIDDEN));
        }

        if (!$request->isMethod('POST')) {
            return $this->render('@SalixCms/install/index.html.twig', [
                'errors' => [],
        ...
        if ([] === $errors) {
            try {
                ...
                return $this->render('@SalixCms/install/success.html.twig', [
